...
TextureCubemap::TextureCubemap(const std::unordered_map<CubeSide, std::string>& side_source, bool vertical_flip)
    : Texture(0, 0, 0), m_textures_paths(side_source), m_vertical_flip(vertical_flip)
{
...
void TextureCubemap::LoadTextureForSide(CubeSide side, CubePosition position)
{
    ...
    stbi_set_flip_vertically_on_load(m_vertical_flip);
